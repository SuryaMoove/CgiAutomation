...
public class HomePageTest extends Reporter {

    //assigning the driver path to a variable
    private final static String driverPath = "C:\\Users\\surya\\Downloads\\chromedriver-win64\\chromedriver-win64\\chromedriver.exe";

    //assigning the home page Url to a variable
    ...
    private static void report() {
        test = reports.startTest("Home Page Test"); //starting test for home page
        test.log(LogStatus.INFO, "Home Page Test Started"); //adding logs and giving status of the log(INFO)
    }

    @BeforeMethod()
    void setUp() {
        System.setProperty("webdriver.chrome.driver", driverPath); //setting the driver path
        System.setProperty("webdriver.http.factory", "jdk-http-client");
        driver = new ChromeDriver(); //initializing the driver

        driver.get(homePageUrl); //navigating to the home page
